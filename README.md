...
- [x] **Phase 6 — Control Flow** *(Branching `if`/`else`, `while`, `for`, short-circuiting logicals `and`/`or`)*
...
| **Control Flow** | Conditionals (`if`/`else`), loops (`while`, `for`), logicals (`and`/`or`) | `PASSED` ✅ |
